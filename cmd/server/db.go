...
func defineDatabaseConnectionFlags(cmd *cobra.Command) (result databaseConnectionFlags) {
	result.dbType = cmd.Flags().StringP("db-type", "T", "", "database type (required). Learn more at https://docs.sqedule.io/server_guide/config/reference/")
	result.dbConnection = cmd.Flags().StringP("db-connection", "D", "", "database connection string. Learn more at https://docs.sqedule.io/server_guide/config/postgresql/")
