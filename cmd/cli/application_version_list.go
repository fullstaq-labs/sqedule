...
// applicationVersionListCmd represents the 'application version list' command
var applicationVersionListCmd = &cobra.Command{
	Use:   "list",
...
func applicationVersionListCmd_run(viper *viper.Viper, printer mocking.IPrinter) error {
	err := applicationVersionListCmd_checkConfig(viper)
	if err != nil {
		return err
	}
	...
	resp, err := req.
		SetResult(&result).
		Get(fmt.Sprintf("/applications/%s/versions",
			url.PathEscape(viper.GetString("application-id"))))
	if err != nil {
		return err
	}
	if resp.IsError() {
		return fmt.Errorf("Error listing application versions: %s", cli.GetApiErrorMessage(resp))
	}
...
func applicationVersionListCmd_checkConfig(viper *viper.Viper) error {
