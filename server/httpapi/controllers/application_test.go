...
var _ = Describe("approval-ruleset API", func() {
	...
	Describe("GET /applications", func() {
		...
		It("outputs all applications", func() {
			...
			Expect(item1["enabled"]).To(Equal(app1.Version.Adjustment.IsEnabled()))
			...
			Expect(item2["enabled"]).To(Equal(app2.Version.Adjustment.IsEnabled()))
	...
	Describe("GET /applications/:id", func() {
		...
		It("outputs the latest version", func() {
			...
			Expect(body["enabled"]).To(Equal(app.Version.Adjustment.IsEnabled()))
