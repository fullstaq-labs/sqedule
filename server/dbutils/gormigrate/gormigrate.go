...
type Options struct {
	...
	// Logger is used for reporting high-level progress.
...
func New(db *gorm.DB, options *Options, migrations []*Migration) *Gormigrate {
	...
	if options.Logger == nil {
		options.Logger = logger.Default.LogMode(logger.Info)
	}
	return &Gormigrate{
		db:         db,
		options:    options,
...
func (g *Gormigrate) runMigration(migration *Migration) error {
	...
	if !migrationRan {
		g.logInfo("Running migration: %s", migration.ID)
...
func (g *Gormigrate) createMigrationTableIfNotExists() error {
	g.logInfo("Ensuring migration table exists")
...
func (g *Gormigrate) logInfo(format string, args ...interface{}) {
	g.options.Logger.Info(context.Background(), format, args...)
}

func (g *Gormigrate) logError(format string, args ...interface{}) {
	g.options.Logger.Error(context.Background(), format, args...)
}
